...
export function passwordAssignedTemplate({ appName, fullName, email, password, loginUrl }) {
	return `
	<div style="font-family: Arial, sans-serif; line-height:1.6; max-width: 600px;">
		<h2>${appName || "Our App"} - Your Password</h2>
		<p>Hi ${fullName || email || "User"},</p>
		<p>Your account password has been generated by an administrator.</p>
		<p>
			<strong>Email:</strong> ${email}<br/>
			<strong>Password:</strong> ${password}
		</p>
		${loginUrl ? `<p style="text-align:center; margin: 24px 0;"><a href="${loginUrl}" style="background:#2563eb; color:#fff; padding:10px 16px; border-radius:6px; text-decoration:none;">Login</a></p>` : ""}
		<p>For your security, please change your password after you log in.</p>
		<p>Thanks,<br/>The ${appName || "App"} Team</p>
	</div>`;
}
